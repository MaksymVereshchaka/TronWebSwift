...
extension secp256k1 {
    ...
    public static func signForRecovery(hash: Data, privateKey: Data, useExtraEntropy: Bool = false, useExtraVer: Bool = true) -> (serializedSignature:Data?, rawSignature: Data?) {
        ...
        for _ in 0...1024 {
            ...
            guard let serializedSignature = serializeSignature(recoverableSignature: &recoverableSignature, useExtraVer: useExtraVer) else {continue}
    ...
    public static func generatePrivateKey() -> Data? {
        for _ in 0...1024 {
            guard let keyData = randomBytes(length: 32) else {
                continue
            }
            guard verifyPrivateKey(privateKey: keyData) else {
                continue
            }
            return keyData
        }
